...
const LanguageTranslator = () => {
  ...
  const translateText = async () => {
    const apiKey = 'AIzaSyBLp7rRmh2IjcvizaEgorw6Wrh3xAma48A';
    const url = `https://translation.googleapis.com/language/translate/v2?key=${apiKey}`;
    localStorage.setItem('IndexCarousel', 5)
    ...
    try {
      ...
      let formattext = decodeHtmlEntities(response.data.data.translations[0].translatedText)
      setTranslatedText(formattext)
    } catch (error) {
      alert('Error ao traduzir o texto! ', error)
    } 
    setLoading(false);
  };

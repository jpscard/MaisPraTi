...
const MovieSearchEngine = () => {
  ...
  localStorage.setItem('IndexCarousel', 2)
  ...
  const searchMovies = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=66454b59bba5c852cc6095ba5746725e&query=${query}&language=pt-BR&page=1`); // Faz uma requisição GET para a API OMDB
  ...
  return (
    <Container>
      ...
      <MoviesContainer>
      {movies && movies.length > 0 ? ( movies.map((movie) => (<MovieCard key={movie.id}>
